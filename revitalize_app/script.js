  ...
  if (document.body.classList.contains('report')) {
    ...
    if (reportEl) {
      ...
      if (!plans || plans.length === 0) {
        ...
        // Ao invés de comparar sempre com o plano imediatamente anterior, mantemos
        // um registro do último peso informado para cada posição de exercício em cada
        // dia (treino). Assim, se um plano intermediário não contiver um peso para
        // determinado exercício, a comparação acontecerá com o último valor
        // conhecido, permitindo detectar progressões ou regressões mesmo após
        // planos de manutenção ou planos sem carga definida.
        const loadDiffs = [];
        // Mapa para armazenar o último peso e o índice do plano onde ele foi registrado
        const lastWeights = {};
        sortedPlans.forEach((plan, planIndex) => {
          (plan.trainings || []).forEach((training, tIdx) => {
            (training.exercises || []).forEach((ex, eIdx) => {
              ...
              const key = `${tIdx}-${eIdx}`;
              if (!isNaN(currW)) {
                // Existe um valor anterior para esta posição? Se sim, calcula a diferença
                if (lastWeights[key] && !isNaN(lastWeights[key].weight) && currW !== lastWeights[key].weight) {
                  loadDiffs.push({
                    from: lastWeights[key].planIndex + 1,
                    to: planIndex + 1,
                    day: tIdx + 1,
                    exercise: ex.name || `Exercício ${eIdx + 1}`,
                    diff: currW - lastWeights[key].weight
                  });
                }
                // Atualiza o valor mais recente desta posição
                lastWeights[key] = { weight: currW, planIndex: planIndex };
              }
            });
          });
        });
